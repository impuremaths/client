...
function encodeNanoBase32(view) {
  ...
  if (bits > 0) output += alphabet[(value << (5 - (bits + offset))) & 31]
...
function decodeNanoBase32(input) {
	...
	if (bits > 0) output[index++] = (value << (bits + offset - 8)) & 255;
...
function normalizeInput (input) {
  ...
  else throw new Error('Input must be an string, Buffer or Uint8Array');
  return ret;
}
